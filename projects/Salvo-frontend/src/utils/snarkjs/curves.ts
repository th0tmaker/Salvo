...
import { Scalar, buildBls12381, buildBn128 } from 'ffjavascript'
...
export async function getCurveFromR(r: any, options?: { singleThread?: boolean }) {
  let curve
...
export async function getCurveFromQ(q: any, options?: { singleThread?: boolean }) {
  let curve
  const singleThread = options && options.singleThread
...
export async function getCurveFromName(name: string, options?: { singleThread?: boolean }) {
  let curve
  const singleThread = options && options.singleThread
